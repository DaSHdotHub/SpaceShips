...
class SpaceShipsGame:
    ...
    def place_spaceship(self, battlefield, style):
        ...
        def computer_turn(battlefield, self):
            ...
            while (
                ...
                if result == "hit":
                    ...
                    if self.computer_turn_data["total_hits"] == self.number_of_ship_segments:
        ...
        def user_turn(battlefield, self):
            ...
            while (
                missiles_fired < NUMBER_OF_MISSILES
                and self.turn_data["total_hits"] < self.number_of_ship_segments
            ):
                row, col = turn_validated_input(battlefield, self.turn_data)
                result = fire_missile(battlefield, (row, col), GREEN_WHITE_STYLE)
                missiles_fired += 1
                if result == "hit":
                    self.turn_data["total_hits"] += 1

                if self.turn_data["total_hits"] == self.number_of_ship_segments:
                    print("All enemy ships have been hit!")
                    break


        def place_spaceship(battlefield, self, style):
            """
            Places an 'L' shaped spaceship on the battlefield. Each spaceship occupies
            3 fields, with one central and side fields forming the 'L' shape. Function
            ensures that the spaceship does not overlap with existing ships and fits
        ...
        def setup_battlefields(self, number_of_ships):
            """
            ...
            """
            user_battlefield = create_battlefield(self.size)
            computer_battlefield = create_battlefield(self.size)

            for _ in range(number_of_ships):
                place_spaceship(user_battlefield, self.size, GREEN_WHITE_STYLE)
                place_spaceship(computer_battlefield, self.size, RED_WHITE_STYLE)
